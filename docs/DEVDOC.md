...
**Die Schrifttabelle wird gerechnet.** `compose/ananeo-schrift.module` (1 073 Einträge unter `𝔵`: 1 047 Zeichen und 26 Makrozeilen) entsteht aus `source/schriftvarianten.d` über `ananeo-tool compose gen-schrift`; ein Wächtertest hält Datei und Rechnung zusammen. Die Notation jeder Zeile ist `<U1D535> <Familienbuchstabe> [<slash>] <Basis> : "Zeichen" UXXXX` – der Familienbuchstabe groß geschrieben heißt fett, ein `/` vor der Basis heißt kursiv (`a` Antiqua, `s` Schreibschrift, `r` Fraktur, `d` doppelt gestrichen, `l` serifenlos, `m` dicktengleich). Ein siebter Familienbuchstabe fällt aus dieser Notation: `c` ist keine Familie, sondern die **Chancery-Variante der Schreibschrift** – dieselben 26 Großbuchstaben mit dem Variantenselektor U+FE00 dahinter (`𝔵 c A` → `𝒜︀`, zwei Codepunkte, beide hinter dem String genannt). Unicode führt nur einen Script-Block; ob `𝒜` als `\mathcal` oder `\mathscr` erscheint, ist Schriftsache, und die standardisierten Variantenfolgen (seit Unicode 14, `data/StandardizedVariants.txt`) gibt es nur für die Großbuchstaben – deshalb keine Kleinbuchstaben. Ein eigener Wachhund hält die Reihe exakt gegen die 26 `chancery style`-Zeilen der Datei. Das große `C` – nach der Notation »Chancery fett«, was es nicht gibt – tippt stattdessen das **LaTeX-Makro als Text**: `𝔵 C A` → `\mathcal{A}`. Grund: KaTeX (gemessen 0.16.11 und 0.18.5) setzt jedes Unicode-Script-Zeichen als `\mathscr` und ignoriert den Selektor; in KaTeX-Feldern führt zu `\mathcal` nur das Makro. Im XCompose-String steht der Backslash als `\\`. Zeichen stehen als `<UXXXX>` statt als benannte Keysyms, damit die Datei nicht an `keysymdef.h` hängt und Latein, Griechisch und die Variantenformen dieselbe Form haben.
